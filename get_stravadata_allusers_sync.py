...
logging.basicConfig(filename='/volume2/web/AilonaTuner/logs/strava_sync_allusers.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
cursor.execute("SELECT id, strava_token, refresh_token, first_name, last_activity_sync, profile_pic FROM users")
...
for user in users:
    ...
    print(f"sycing data for user {user_id}")
    


    # Convert date to datetime at midnight
    last_activity_sync = datetime.datetime.combine(
        last_activity_sync, datetime.time())

    # Get the activities since the last service
    response = requests.get(
        "https://www.strava.com/api/v3/athlete/activities",
        headers={"Authorization": f"Bearer {strava_token}"},
        params={"after": last_activity_sync.timestamp(), "per_page": 200}
    )
